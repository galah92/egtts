"""
Milestone 2: EXPLAIN-Guided Beam Search Evaluation

Evaluates beam search + EXPLAIN selection on Spider dataset.
Generates multiple candidates and picks first EXPLAIN-valid one.
"""
...
def evaluate_milestone2(num_examples: int = 100, num_beams: int = 5):
    """
    Run Milestone 2 beam search evaluation.
    ...
    """
    print("=" * 80)
    print("MILESTONE 2: EXPLAIN-GUIDED BEAM SEARCH")
    print("=" * 80)
    ...
    print(f"\n{'=' * 80}")
    print(f"Evaluating {num_examples} examples with {num_beams} beams")
    print(f"{'=' * 80}\n")
    ...
    for idx, example in enumerate(examples):
        ...
        # Generate with beam search + EXPLAIN selection
        result = generator.generate_with_beam_search(question, schema, str(db_path), num_beams=num_beams)
    ...
    initially_invalid = total - beam_success[0]
    fixed_count = sum(beam_success[i] for i in range(1, num_beams + 1))
