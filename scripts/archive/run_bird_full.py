"""
BIRD Full Benchmark: Run on all 500 Mini-Dev instances.

BIRD focuses on VES (Valid Efficiency Score) which evaluates:
1. Execution accuracy (correctness)
2. Query efficiency (via EXPLAIN QUERY PLAN cost)

Supports --baseline and --strategy flags like run_spider_full.py
"""
...
def run_bird_full_benchmark(baseline: bool = False, strategy: str = "M4"):
    """
    Run full BIRD benchmark on all 500 Mini-Dev instances.
    ...
    print("=" * 80)
    print(f"BIRD FULL BENCHMARK: {method}")
    print("=" * 80)

    # Load BIRD data
    bird_data_file = Path("data/minidev/MINIDEV/mini_dev_sqlite.json")
    db_base_dir = Path("data/minidev/MINIDEV/dev_databases")

    print(f"\nLoading BIRD mini_dev_sqlite.json...")
    with open(bird_data_file) as f:
    ...
    for idx, example in enumerate(tqdm(examples, desc="Generating SQL")):
        ...
        # Get database path
        db_path = db_base_dir / db_id / f"{db_id}.sqlite"
    ...
    # Save results in BIRD evaluation format (one SQL per line)
    if baseline:
        suffix = "baseline"
    elif strategy == "M3":
        suffix = "m3"
    ...
    output_file = Path(f"bird_results_{suffix}.txt")
    with open(output_file, "w") as f:
        for sql in results:
            # BIRD evaluation expects one query per line
    ...
    metadata_file = Path(f"results/bird_full_metadata_{suffix}.json")
    ...
    print(f"\n{'=' * 80}")
    print("Next steps:")
    print("1. Use BIRD evaluation script to compute VES (Valid Efficiency Score)")
    print("2. Compare efficiency scores: Baseline vs M3 vs M4")
    print(f"{'=' * 80}")


if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Run BIRD full benchmark")
    parser.add_argument(
        "--baseline",
        action="store_true",
    ...
    run_bird_full_benchmark(baseline=args.baseline, strategy=args.strategy)
