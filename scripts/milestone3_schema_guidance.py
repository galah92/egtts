"""
Milestone 3: Token-Level Schema Guidance Evaluation

Evaluates Strategy A (Schema Lookup) from the guidance.
Uses LogitsProcessor to prevent schema hallucinations during generation.
"""
...
def evaluate_milestone3(num_examples: int = 100, num_beams: int = 5):
    """
    Run Milestone 3 schema guidance evaluation.
    ...
    """
    print("=" * 80)
    print("MILESTONE 3: TOKEN-LEVEL SCHEMA GUIDANCE (Strategy A)")
    print("=" * 80)
    ...
    # Evaluation metrics
    results = []
    beam_success = defaultdict(int)  # Success count at each beam position
    error_types = defaultdict(int)
    fixed_errors = defaultdict(int)
    persistent_errors = []

    print(f"\n{'=' * 80}")
    print(f"Evaluating {num_examples} examples with {num_beams} beams + schema guidance")
    print(f"{'=' * 80}\n")
    ...
    for idx, example in enumerate(examples):
        ...
        if execution_correct:
            beam_success[result.iterations] += 1
            if result.iterations == 0:
                print(f"  ✓ Correct with beam 0 ({result.latency_ms:.0f}ms)")
            else:
                print(f"  ✓ Correct with beam {result.iterations} ({result.latency_ms:.0f}ms)")
            ...
            if result.iterations == 0:
                ...
                print(f"  ✗ Still incorrect after {result.iterations} beams ({result.latency_ms:.0f}ms)")
    ...
    total = len(results)
    baseline_accuracy = beam_success[0] / total * 100
    final_accuracy = sum(beam_success.values()) / total * 100
    improvement = final_accuracy - baseline_accuracy

    print("\nExecution Accuracy (Benchmark Metric):")
    print(f"  Baseline (iteration 0): {beam_success[0]}/{total} ({baseline_accuracy:.1f}%)")
    print(f"  Final (after refinement): {sum(beam_success.values())}/{total} ({final_accuracy:.1f}%)")
    print(f"  Improvement: +{improvement:.1f}%")

    print("\nSuccess by Beam:")
    for i in range(num_beams + 1):
        count = beam_success[i]
        pct = count / total * 100 if total > 0 else 0
        print(f"  Beam {i}: {count}/{total} ({pct:.1f}%)")

    # Average beams for correct queries
    ...
    if correct_results:
        ...
        print(f"\nAverage beams (correct): {avg_iterations:.2f}")
    ...
    print(f"  Execution correct: {sum(beam_success.values())}/{total} ({final_accuracy:.1f}%)")
    print(f"  Gap (valid but wrong): {explain_valid - sum(beam_success.values())}")
    ...
    schema_errors_beam0 = sum(
        1
        for r in results
        if r["error_history"]
        and any("no such column" in e.lower() or "no such table" in e.lower() for e in [r["error_history"][0]])
    )
    print(f"\nSchema Hallucinations in Beam 0: {schema_errors_beam0}/{total}")
    print("  (Should be 0 or very low if schema guidance works)")

    # Save results
    output_file = Path("results/milestone3_evaluation.json")
    ...
    summary = {
        ...
        "iteration_breakdown": {f"iter_{i}": beam_success[i] for i in range(num_beams + 1)},
        "latency": {
        ...
        "schema_hallucinations_beam0": schema_errors_beam0,
    }
...
if __name__ == "__main__":
    ...
    evaluate_milestone3(num_examples=num_examples)
