...
def run_evaluation(strategy: str, metric: str = "all"):
    ...
    db_root = (
        str(project_root / "data" / "mini_dev_official" / "sqlite" / "dev_databases")
        + "/"
    )
    ground_truth = (
        project_root
    ...
    diff_json = (
        project_root / "data" / "mini_dev_official" / "sqlite" / "mini_dev_sqlite.jsonl"
    )
    ...
    if not pred_path.exists():
        ...
        print("Run: python3 scripts/convert_to_official_format.py \\")
    ...
    if not eval_dir.exists():
        ...
        print(
            "Run: git clone https://github.com/bird-bench/mini_dev.git data/mini_dev_official"
        )
        sys.exit(1)
